...
tar czf /tmp/vp-deploy.tar.gz -C /root \
  ...
  --exclude='voucher-platform/web-internal' \
  voucher-platform
...
build_backend() {
  ...
  CTEX 'cd /opt/voucher-platform && npm install --no-audit --no-fund >/dev/null 2>&1 \
...
build_front() { # $1=dir $2=pm2name
  ...
  CTEX "cd /opt/voucher-platform/$1 && npm install --no-audit --no-fund >/dev/null 2>&1 && npm run build >/dev/null 2>&1"
