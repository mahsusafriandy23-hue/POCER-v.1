...
export async function api<T = unknown>(path: string, opts: Options = {}): Promise<T> {
  ...
  try {
    res = await fetch(`${API_BASE}${path}`, {
      ...
      signal,
      // Always hit the network — outlets/packages/balance change server-side
      // (owner reassigns outlets, tops up, etc.) and must never be served stale
...
export type AgentPkg = {
  ...
  name: string;
  price: number;
  agentPrice: number; // the agent's COST (set by owner)
  sellPrice: number; // the agent's own selling price to the customer
  margin: number; // sellPrice − agentPrice
  customSellPrice: boolean; // true when the agent set a custom price (vs default retail)
  duration: string;
  isActive: boolean;
  server: Outlet | null;
};
...
export type ChartDay = { day: number; count: number; revenue: number };
export type ServerStatus = { id: number; code: string; name: string; online: boolean; latencyMs?: number; error?: string };

export const agent = {
  me: () => api<AgentProfile>("/agent/me"),
  stats: () => api<AgentStats>("/agent/stats"),
  chart: () => api<ChartDay[]>("/agent/chart"),
  serverStatus: () => api<ServerStatus[]>("/agent/server-status"),
  outlets: () => api<(Outlet & { isActive: boolean })[]>("/agent/outlets"),
  packages: (serverId?: number) =>
    api<AgentPkg[]>(`/agent/packages${serverId ? `?server=${serverId}` : ""}`),
  setSellPrice: (packageId: number, sellPrice: number) =>
    api<SellPriceResult>(`/agent/packages/${packageId}/sell-price`, {
      method: "PUT",
      body: { sellPrice },
    }),
  ...
  lookupCustomer: (handle: string) =>
    api<{ found: boolean; id?: number; name?: string | null; username?: string | null; phone?: string }>(
      `/agent/customer-lookup?handle=${encodeURIComponent(handle)}`,
    ),
  sellToCustomer: (body: { username?: string; customerId?: number; packageId: number; pin?: string }) =>
  ...
  sellBatch: (body: {
    ...
    pin?: string;
  }) => api<{ count: number; results: SellResult[] }>("/agent/sell-batch", { method: "POST", body }),
  contacts: {
    list: () => api<AgentContact[]>("/agent/contacts"),
    create: (body: { name: string; username?: string; phone?: string; note?: string }) =>
      api<AgentContact>("/agent/contacts", { method: "POST", body }),
    update: (id: number, body: { name?: string; username?: string; phone?: string; note?: string }) =>
      api<AgentContact>(`/agent/contacts/${id}`, { method: "PATCH", body }),
    delete: (id: number) => api<{ deleted: boolean }>(`/agent/contacts/${id}`, { method: "DELETE" }),
  },
};
