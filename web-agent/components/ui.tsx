...
export function Button({
  ...
  const variants: Record<string, string> = {
    primary: "bg-azure text-[#0A0A0F] shadow-qa hover:bg-azure-400 active:scale-[.98] font-extrabold",
    soft: "bg-haze text-azure hover:bg-[#222232] active:scale-[.98]",
...
export const TextField = forwardRef<HTMLInputElement, FieldProps>(function TextField(
  ...
  return (
    <label className="block">
      ...
      <div className="flex items-center gap-2 rounded-2xl bg-surface border border-line px-4 focus-within:border-azure focus-within:ring-2 focus-within:ring-azure/20 transition-all">
...
export function Alert({ tone = "error", children }: { tone?: "error" | "info" | "success"; children: ReactNode }) {
  const tones: Record<string, string> = {
    error: "bg-rose-950/60 text-rose-300 border-rose-900",
